...
if nargin < 1
    ...
    CFG.PREWIN  = 0.2; % Add this winlen prior the templates
...
%idD=[3:numel(DDIR)]; % put here DDIR id to be removed
%DDIR(idD)=[];
% otherwise put here directory to process
idD=[301:330]; % put here DDIR id to process
DDIR=DDIR(idD);
        ...
        if numel(p)==0     %% se è 0 vuol dire che non è stato creato prima -> REFRESH=1
            try
                ...
                while numel(p)==0 % p.Connected==0;
                    fprintf('Noway..!! Trying to wake up parallel pool\n');
                    p=parpool(CFG.NWORK);
                end
            end
        end
